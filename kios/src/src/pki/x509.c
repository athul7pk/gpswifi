...
*/
	if (trustedCAFiles != NULL) {
		if (matrixX509ReadCert(pool, trustedCAFiles, &caCert, &caCertLen,
				&chain) < 0 || caCert == NULL) {
			/*matrixStrDebugMsg*/fprintf(stdout,"Error reading CA cert files %s\n",
				(char*)trustedCAFiles);
			matrixRsaFreeKeys(lkeys);
			return -1;
		}
							...
							sslRsaCert_t **subjectCert, int32 *valid)
{
	...
	*valid = 1;
	while ((*subjectCert)->next != NULL) {
		ic = (*subjectCert)->next;
		//if (matrixX509ValidateCertInternal(pool, *subjectCert, ic, 1) < 0) { //shinaj
		//	*valid = -1;
		//	return -1;
		//}
/*
...
{
	//if (matrixX509ValidateCertInternal(pool, subjectCert, issuerCert, 0) < 0) { //shinaj
	//	*valid = -1;
	//	return -1;
	//}
